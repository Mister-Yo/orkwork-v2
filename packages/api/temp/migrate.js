...
async function runMigrations() {
    try {
        ...
        await index_1.db.execute((0, drizzle_orm_1.sql) `CREATE EXTENSION IF NOT EXISTS "uuid-ossp"`);
        await index_1.db.execute((0, drizzle_orm_1.sql) `CREATE EXTENSION IF NOT EXISTS vector`);
        // Create enums
        ...
        for (const e of enums) {
            await index_1.db.execute(drizzle_orm_1.sql.raw(e));
        }
        // Create tables
    ...
    `);
        ...
        for (const stmt of alterStatements) {
            await index_1.db.execute(drizzle_orm_1.sql.raw(stmt));
        }
    ...
    `);
        ...
        for (const idx of indexes) {
            await index_1.db.execute(drizzle_orm_1.sql.raw(idx));
        }
